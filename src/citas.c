...
void generar_reporte_citas_completo() {
	...
	fprintf(f_txt, "****************************************************************************************\n");
	fprintf(f_txt, "%-4s | %-12s | %-7s | %-10s | %-12s | %-30s | %s\n",
			"ID", "FECHA", "HORA", "PACIENTE", "MEDICO", "MOTIVO", "ESTADO");
	fprintf(f_txt, "==========================================================================================\n");
	
	for (int i = 0; i < total_citas; i++) {
		const char *estado_str = obtener_estado_str(db_citas[i].estado);
		fprintf(f_txt, "%-4d | %-12s | %-7s | %-10s | %-10s | %-30s | %s\n",
				db_citas[i].id,
				db_citas[i].fecha,
				db_citas[i].hora,
				db_citas[i].id_paciente,
	...
	if (f_html) {
		...
		fprintf(f_html, "    <tr><th>ID</th><th>Fecha</th><th>Hora</th><th>Paciente</th><th>Medico</th><th>Motivo</th><th>Estado</th></tr>\n");
