...
int main() {
	...
	printf("\n [i] Generando BD Masiva (Curva Estacional Realista y Porcentajes Perfectos)...\n");
	...
	// Estos pesos suman 100 y dictan la forma exacta de tu curva gráfica
	int pesos_curva[12] = {4, 5, 6, 7, 9, 11, 14, 15, 10, 7, 6, 6}; 
	
	while(citas_generadas < NUM_CITAS) {
		
		// ====================================================================
		// LÓGICA DE CURVA REALISTA: Crece, Pico Máximo, Baja moderado y se mantiene
		// ====================================================================
		...
		// Convertimos el mes elegido (0 a 11) en días de desplazamiento (bloques de 30 días)
		...
		if (doctor_ocupado[d_idx][dias_desplazamiento][slot] == 0) {
			...
			// ====================================================================
			// GENERACIÓN ESTADÍSTICA EXACTA GLOBAL (78%, 12%, 6%, 4%)
			// ====================================================================
			int estado;
			int prob = rand() % 10000; 
			...
			int p_idx;
			if (estado == 3) {
				p_idx = rand() % 300; // Inasistencias se concentran en 300 pacientes para multas altas
			} else {
	...
	printf("\n === FORMA DE LA CURVA GENERADA (Auditoria) ===\n");
	for (int m = 0; m < 12; m++) {
		printf(" Mes %02d de proyeccion: %d citas\n", m+1, contador_curva_meses[m]);
	}
