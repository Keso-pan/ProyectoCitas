...
#define MULTA_INASISTENCIA 25.0
...
void solicitar_cita(const char *mi_cedula) {
	...
	} else if (strcmp(esp_elegida, "Gastroenterologia") == 0) {
		op1 = "Dolor abdominal"; op2 = "Reflujo gastrico"; op3 = "Control de gastritis";
	} else if (strcmp(esp_elegida, "Neumologia") == 0) {
		op1 = "Dificultad al respirar"; op2 = "Tos cronica"; op3 = "Crisis de Asma";
	} else if (strcmp(esp_elegida, "Endocrinologia") == 0) {
		op1 = "Control de diabetes"; op2 = "Problemas de tiroides"; op3 = "Desorden hormonal";
	} else if (strcmp(esp_elegida, "Otorrinolaringologia") == 0) {
		op1 = "Dolor de oido"; op2 = "Sinusitis severa"; op3 = "Problemas de garganta";
	} else if (strcmp(esp_elegida, "Nefrologia") == 0) {
		op1 = "Control renal"; op2 = "Infeccion urinaria"; op3 = "Calculos renales";
	}
...
void generar_reporte_general() {
	...
	int activas = 0, canceladas = 0, inasistencias = 0;
	float recaudacion = 0.0;
	
	for(int i=0; i<total_citas; i++) {
		if(db_citas[i].estado == 0 || db_citas[i].estado == 2) activas++;
		else if(db_citas[i].estado == 1) canceladas++;
		else if(db_citas[i].estado == 3) {
			inasistencias++;
			recaudacion += 25.0;
		}
	}
	...
	fprintf(f, " --------------------------------------------\n");
	fprintf(f, " [v] Citas Activas/Atendidas:    %d\n", activas);
	fprintf(f, " [x] Citas Canceladas:           %d\n", canceladas);
	fprintf(f, " [!] Inasistencias (No Asistio): %d\n", inasistencias);
	fprintf(f, "============================================\n");
	fprintf(f, " RECAUDACION POR INASISTENCIA:   $%.2f\n", recaudacion);
	fprintf(f, "============================================\n");
...
void generar_reporte_citas_completo() {
	...
	fprintf(f_txt, "Generado: %d-%d-%d %d:%02d:%02d\n\n",
			...
			fecha_actual->tm_hour, fecha_actual->tm_min, fecha_actual->tm_sec);
	...
	fprintf(f_txt, "Inasistencias:       %d (%.1f%%)\n", inasistencias, (float)inasistencias/total_citas*100);
	fprintf(f_txt, "----------------------------------------------------------------------------\n");
	fprintf(f_txt, "RECAUDACION TOTAL POR INASISTENCIAS: $%.2f\n\n", total_recaudado);
	...
	for (int i = 0; i < total_citas; i++) {
		...
		float multa_cita = (db_citas[i].estado == 3) ? 25.0 : 0.0;
		
		fprintf(f_txt, "%-4d | %-12s | %-8s | %-10s | %-12s | %-25s | %-10s | $%-7.2f\n",
				db_citas[i].id, db_citas[i].fecha, db_citas[i].hora, db_citas[i].id_paciente,
				db_citas[i].codigo_medico, db_citas[i].motivo, estado_str, multa_cita);
	}
	...
	if (f_html) {
		...
		fprintf(f_html, "  <style>\n    body { font-family: Arial, sans-serif; margin: 20px; }\n    h1 { color: #1E5AAE; text-align: center; }\n    table { border-collapse: collapse; width: 100%%; }\n    th, td { border: 1px solid #ddd; padding: 8px; text-align: left; }\n    th { background-color: #2E75B6; color: white; }\n    .estadistica { background-color: #f9f9f9; padding: 10px; margin: 10px 0; }\n    .recaudacion { color: #d9534f; font-size: 18px; font-weight: bold; }\n  </style>\n</head>\n<body>\n");
		...
		fprintf(f_html, "    <strong>Citas activas/atendidas:</strong> %d (%.1f%%)<br>\n", activas, (float)activas/total_citas*100);
		fprintf(f_html, "    <strong>Citas canceladas:</strong> %d (%.1f%%)<br>\n", canceladas, (float)canceladas/total_citas*100);
		fprintf(f_html, "    <strong>Inasistencias:</strong> %d (%.1f%%)<br><br>\n", inasistencias, (float)inasistencias/total_citas*100);
		fprintf(f_html, "    <span class='recaudacion'>Recaudación por Inasistencias: $%.2f</span>\n", total_recaudado);
		fprintf(f_html, "  </div>\n");
		
		fprintf(f_html, "  <table>\n    <tr><th>ID</th><th>Fecha</th><th>Hora</th><th>Paciente</th><th>Medico</th><th>Motivo</th><th>Estado</th><th>Multa</th></tr>\n");
...
void generar_reportes_mis() {
	...
	int efectivas = 0, canceladas = 0, inasistencias = 0;
	float total_recaudado = 0.0;
	
	for(int i = 0; i < total_citas; i++) {
		if(db_citas[i].estado == 0 || db_citas[i].estado == 2) efectivas++;
		else if(db_citas[i].estado == 1) canceladas++;
		else if(db_citas[i].estado == 3) {
			inasistencias++;
			total_recaudado += 25.0;
	...
	int total_evaluadas = efectivas + canceladas + inasistencias;
	...
	float pct_inasistencias = (total_evaluadas > 0) ? ((float)inasistencias / total_evaluadas) * 100 : 0.0;
	
	printf(" %-20s | %-10d | %.1f%%\n", "Activa/Atendida", efectivas, pct_efectivas);
	printf(" %-20s | %-10d | %.1f%%\n", "Cancelada", canceladas, pct_canceladas);
	printf(" %-20s | %-10d | %.1f%%\n", "No Asistio", inasistencias, pct_inasistencias);
	printf(" --------------------------------------------------\n\n");
	...
	printf(" [D] REPORTE FINANCIERO (MULTAS POR INASISTENCIA):\n");
	printf(" %-25s | %-20s\n", "Concepto", "Monto/Cantidad");
	printf(" ***************************************************\n");
	printf(" %-25s | %-20d\n", "Total de Inasistencias", inasistencias);
	printf(" %-25s | $%-19.2f\n", "Recaudacion Generada", total_recaudado);
	printf(" ***************************************************\n\n");
